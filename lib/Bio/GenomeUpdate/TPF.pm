...
sub get_tpf_with_bacs_inserted {
  ...
  foreach my $bac_ref (@bacs) {
	...
	if ($prev_line_key && $bac_start <= $prev_agp_end && $bac_end >= $agp_start && $tpf_lines{$line_key-1}->get_line_type() eq 'gap') {
	...
	if ($prev_line_key && $bac_start < $agp_start && $bac_start > $prev_agp_end && $tpf_lines{$line_key-1}->get_line_type() eq 'gap') {
	  $gaps_to_resize{$line_key-1} = $bac_start - $prev_agp_end;
	}
	if ($prev_line_key && $bac_end < $agp_start && $bac_end > $prev_agp_end && $tpf_lines{$line_key-1}->get_line_type() eq 'gap') {
...
sub move_scaffold_before {
  ...
  foreach my $line_key (@sorted_line_numbers_in_copy) {
    $self->insert_line_after($line_number_to_insert_before-1,$lines_in_copy{$line_key});
...
sub flip_scaffold {
  ...
  #remove lines in range
  foreach my $line_key (reverse($first_line_number..$last_line_number-1)) {
    $self->delete_line($line_key);
  }
  $line_number_to_insert_before = $first_line_number-1;
  my %lines_in_copy = %{$tpf_copy->get_tpf_lines()};
  my @sorted_line_numbers_in_copy = reverse(sort { $a <=> $b } keys %lines_in_copy);
  foreach my $line_key (@sorted_line_numbers_in_copy) {
    $self->insert_line_after($line_number_to_insert_before,$lines_in_copy{$line_key});
    $line_number_to_insert_before++;
    $last_line++;
  }
}
