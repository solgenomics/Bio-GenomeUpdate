...
=head1 DESCRIPTION
    ...
    This package performs calculations on an array of objects containing alignment coordinates (e.g., from Nucmer show-coords output) for a single query sequence (e.g., from a BAC) relative to multiple reference sequences (e.g., chromosome pseudomolecules).
...
sub get_cluster_groups_sorted_by_length {
	...
	sub calculate_length {
		...
		$length += (($align_coords->get_reference_end_coord()+1) - $align_coords->get_reference_start_coord());
		if ($prev_end_coord){
		    if  ($align_coords->get_reference_start_coord() <= $prev_end_coord){
			$length -= (($prev_end_coord+1) - $align_coords->get_reference_start_coord()); #subtract overlap between alignment clusters from total alignment length
		    }
		}
...
Returns IDs, start and end coordiantes, total aligned sequence length, and direction for the longest proximity-grouped alignment clusters sorted by longest to shortest length of non-overlapping sequence covered by alignment clusters.  The proximity grouping is done using the specified length of an allowed gap between aligned clusters in the reference sequence ($gap allowed) plus an adjustment based on a percent of the length of the gap between the aligned clusters on the query sequence. 
...
sub get_id_coords_and_direction_of_longest_alignment_cluster_group {
	...
	my $curr_ref_start = $current_cluster_group[0]->get_reference_start_coord();
	my $curr_ref_end = $current_cluster_group[-1]->get_reference_end_coord();
	...
	foreach my $align_coords (@current_cluster_group){
	    $sequence_aligned_in_current_cluster += (($align_coords->get_reference_end_coord()+1) - $align_coords->get_reference_start_coord());
	    if ($prev_end_coord_of_current_cluster){
		if  ($align_coords->get_reference_start_coord() <= $prev_end_coord_of_current_cluster){
		    $sequence_aligned_in_current_cluster -= (($prev_end_coord_of_current_cluster+1) - $align_coords->get_reference_start_coord()); #subtract overlap between alignment clusters from total alignment length
		}
	    }
	    $prev_end_coord_of_current_cluster = $align_coords->get_reference_end_coord();
	}
	if (defined($alternates)){
	    $alternates = $alternates.", ".$curr_ref_id.":".$sequence_aligned_in_current_cluster.":".$curr_ref_start.":".$curr_ref_end;
	}
	else {
	    $alternates = $curr_ref_id.":".$sequence_aligned_in_current_cluster.":".$curr_ref_start.":".$curr_ref_end;
	}
    }
