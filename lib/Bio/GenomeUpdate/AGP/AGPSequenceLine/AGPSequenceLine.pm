...
=head1 NAME
    ...
    AGPSequenceLine - A sequence line in an Accessioned Golden Path (AGP)

=head1 SYNOPSIS

    my $variable = AGPSequenceLine->new();

=head1 DESCRIPTION

    This class stores information for a sequence line in an Accessioned Golden Path (AGP).
...
subtype 'AGPSequenceComponentType',
    as 'Str',
    where {$_ eq "A" || $_ eq "D" ||  $_ eq "F" || $_ eq "G" || $_ eq "O" || $_ eq "P" || $_ eq "W"};
has 'component_type' => (isa => 'AGPSequenceComponentType', is => 'rw', predicate => 'has_component_type');
