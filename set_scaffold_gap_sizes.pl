...
my $gap_info = read_file($gap_input_file) or die "Could not open gap info file: $gap_input_file\n";
...
#  push(@gaps_sizes_and_methods,\@gap_size_and_method_array);
  $tpf->change_gap_size_between_scaffolds($first_scaffold, $second_scaffold, $gap_size, $gap_method);
}
